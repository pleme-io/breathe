...
| 8 | **OutcomeChain attestation** | Every `assign`/`release` appends a BLAKE3+Ed25519 entry over `{target, dimension, used, capacity, from, to, verdict, epoch}`; every Conflict/NoLimit/NoSafeShrink → AnomalyChain. `status.outcomeChainHead` surfaces the head. `kensa verify outcome-chain --target X --dimension memory` proves the band held over a window. **Attest state-changes + a periodic in-band heartbeat, not every Hold** — keeps signing load bounded (a flagged tradeoff, resolved toward bounded). **Implemented 2026-07-26** as `breathe_store::GatedDecisionLog`, a decorator wrapping whichever `DecisionLog` tier `scale.store` selects (so both append paths — `fold_counters` and the replica reconcile — share one definition of "changed"): a tick is appended when it advances a counter, is the band's first this process, differs from the last recorded `DecisionEntry`, or `scale.decisionHeartbeatSeconds` (default 900) have elapsed. Chain integrity is untouched (`seq` contiguous, linkage from genesis, `verify_chain` unchanged) — but a *gap* now means "the band re-decided the same thing", not "the controller did not tick"; absence beyond one heartbeat is the liveness signal. |
