...
use crate::node_forma::{
    CLAIM_POOL_LABEL, demand_attributable, is_busy_runner_pod, node_ready, parse_cpu_milli,
};
...
fn nodepool_resource() -> ApiResource {
    ApiResource::from_gvk_with_plural(
        &GroupVersionKind::gvk(KARPENTER_GROUP, KARPENTER_VERSION, "NodePool"),
        "nodepools",
    )
}
fn nodeclaim_resource() -> ApiResource {
    ApiResource::from_gvk_with_plural(
        &GroupVersionKind::gvk(KARPENTER_GROUP, KARPENTER_VERSION, "NodeClaim"),
        "nodeclaims",
    )
}
...
pub trait KarpenterEnvironment: Send + Sync {
    ...
    /// `EksKarpenter` fleet existing". the private estate now runs six NodePools, so that
    ...
    async fn get_nodepool_template(
        &self,
        node_pool_ref: &str,
    ) -> Result<NodePoolTemplate, ProviderError>;
    /// This pool's own NodeClaims (labeled `CLAIM_POOL_LABEL=pool`).
    async fn list_managed_nodeclaims(&self, pool: &str)
    -> Result<Vec<NodeClaimRef>, ProviderError>;
    ...
    async fn observe_busy_node_names(
        &self,
...
fn claim_is_busy(claim: &NodeClaimRef, busy_node_names: &HashSet<String>) -> bool {
    claim
        .node_name
        .as_deref()
        .is_some_and(|n| busy_node_names.contains(n))
}
...
fn build_nodeclaim(pool: &str, node_pool_ref: &str, template: &NodePoolTemplate) -> DynamicObject {
    ...
    labels.insert(
        KARPENTER_NODE_POOL_REF_LABEL.to_string(),
        node_pool_ref.to_string(),
    );
    labels.insert(
        KARPENTER_NODEPOOL_LABEL.to_string(),
        node_pool_ref.to_string(),
    );

    let mut obj = DynamicObject::new("", &nodeclaim_resource())
        .data(serde_json::json!({ "spec": template.spec }));
...
fn parse_nodepool_template(
    node_pool_ref: &str,
    node_pool_data: &serde_json::Value,
) -> Result<NodePoolTemplate, ProviderError> {
    ...
    let spec = template
        ...
        .ok_or_else(|| {
            ProviderError::ApiPermanent(format!(
                "NodePool {node_pool_ref} has no spec.template.spec"
            ))
        })?;
...
impl<E: KarpenterEnvironment> Provedor for KarpenterProvedor<E> {
    async fn observe(&self) -> Result<FormaSample, ProviderError> {
        ...
        let demand_milli = self
            .env
            .observe_pod_demand_milli(&self.node_pool_ref)
            .await?;
        ...
        // mechanism. Measured on one cluster: both pools pegged Growing with
        // flapDetected=true.
        //
    ...
    async fn provision(&self, n: u64) -> Result<ProvisionReceipt, ProviderError> {
        ...
        for _ in 0..n {
            ...
            match self.env.create_nodeclaim(obj).await {
                Ok(()) => created += 1,
                Err(e) => {
                    warn!(pool = %self.pool, error = %e, "NodeClaim create failed (non-fatal; retried next tick)")
                }
            }
        }
        if created == 0 {
        ...
        Ok(ProvisionReceipt::Applied {
            delta: created,
            plan_id: format!("karpenter:provision:{}", self.pool),
        })
    }
    ...
    async fn deprovision(&self, n: u64) -> Result<ProvisionReceipt, ProviderError> {
        ...
        for claim in claims
            .iter()
            .filter(|c| !claim_is_busy(c, &busy_nodes))
            .take(n as usize)
        {
            ...
            match self.env.delete_nodeclaim(&claim.name).await {
                Ok(()) => released += 1,
                Err(e) => {
                    warn!(pool = %self.pool, claim = %claim.name, error = %e, "NodeClaim delete failed (non-fatal)")
                }
            }
        }
        if released == 0 {
        ...
        Ok(ProvisionReceipt::Applied {
            delta: -released,
            plan_id: format!("karpenter:deprovision:{}", self.pool),
        })
    }
}
...
impl KarpenterEnvironment for KubeKarpenterEnvironment {
    ...
    async fn list_managed_nodeclaims(
        &self,
        pool: &str,
    ) -> Result<Vec<NodeClaimRef>, ProviderError> {
        ...
        Ok(list
            ...
            .map(|o| NodeClaimRef {
                ...
                pool_label: o
                    ...
                    .labels
                    .as_ref()
                    .and_then(|l| l.get(CLAIM_POOL_LABEL))
                    .cloned(),
...
mod tests {
    use super::{
        ...
        KarpenterEnvironment, KarpenterProvedor, NodeClaimRef, NodePoolTemplate, ObservedNode,
        build_nodeclaim, claim_is_busy, is_karpenter_managed_ref, nodeclaim_resource,
        nodepool_resource, owned_by_nodepool, parse_nodepool_template,
    };
    ...
    fn owned_by_nodepool_matches_only_the_referenced_nodepool() {
        assert!(
            !owned_by_nodepool(&node_with_nodepool_label(None), "isolated-agents"),
            "an unlabelled node is never owned"
        );
        assert!(
            !owned_by_nodepool(
                &node_with_nodepool_label(Some("other-pool")),
                "isolated-agents"
            ),
        ...
        assert!(
            owned_by_nodepool(
                &node_with_nodepool_label(Some("isolated-agents")),
                "isolated-agents"
            ),
    ...
    fn is_karpenter_managed_ref_matches_only_this_pool() {
        ...
        let none = NodeClaimRef {
            name: "a".into(),
            pool_label: None,
            node_name: None,
        };
        let other = NodeClaimRef {
            name: "b".into(),
            pool_label: Some("other-pool".into()),
            node_name: None,
        };
        let mine = NodeClaimRef {
            name: "c".into(),
            pool_label: Some("isolated-agents".into()),
            node_name: None,
        };
        assert!(!is_karpenter_managed_ref(&none, "isolated-agents"));
        assert!(!is_karpenter_managed_ref(&other, "isolated-agents"));
        assert!(is_karpenter_managed_ref(&mine, "isolated-agents"));
    }
    ...
    fn build_nodeclaim_has_no_fixed_name_generates_from_pool_and_copies_spec_verbatim() {
        ...
        let template = NodePoolTemplate {
            spec: spec.clone(),
            ..NodePoolTemplate::default()
        };
        let obj = build_nodeclaim("isolated-agents", "isolated-nodepool", &template);

        // No fixed name — the apiserver assigns the suffix (concurrent
        // provisions never collide).
        assert_eq!(
            obj.metadata.name, None,
            "a minted NodeClaim must never carry a fixed name"
        );
        assert_eq!(
            obj.metadata.generate_name.as_deref(),
            Some("breathe-isolated-agents-")
        );

        let labels = obj.metadata.labels.as_ref().expect("labels set");
        assert_eq!(
            labels.get(CLAIM_POOL_LABEL).map(String::as_str),
            Some("isolated-agents")
        );
        assert_eq!(
            labels
                .get(KARPENTER_NODE_POOL_REF_LABEL)
                .map(String::as_str),
            Some("isolated-nodepool")
        );

        // The template spec is copied VERBATIM — no field-by-field re-modeling.
        assert_eq!(
            obj.data["spec"], spec,
            "spec.template.spec is copied verbatim into the NodeClaim spec"
        );
    }
    ...
    fn build_nodeclaim_stamps_the_ownership_label_observe_owned_nodes_actually_filters_on() {
        ...
        let node =
            node_with_nodepool_label(labels.get(KARPENTER_NODEPOOL_LABEL).map(String::as_str));
        assert!(owned_by_nodepool(&node, "isolated-nodepool"));
    }
    ...
    fn build_nodeclaim_copies_template_metadata_labels_and_annotations_without_letting_them_shadow_ownership()
     {
        ...
        template_labels.insert(
            KARPENTER_NODEPOOL_LABEL.to_string(),
            "should-never-survive".to_string(),
        );
        ...
        assert_eq!(
            ...
            Some("platform"),
            "template-authored labels propagate"
        );
        assert_eq!(
            labels.get(KARPENTER_NODEPOOL_LABEL).map(String::as_str),
            Some("isolated-nodepool"),
    ...
    fn build_nodeclaim_with_no_template_metadata_does_not_panic_and_still_stamps_ownership() {
        ...
        assert!(
            ...
            "no annotations block is set on the NodeClaim when the template carries none"
        );
    }
    ...
    fn parse_nodepool_template_with_no_metadata_block_defaults_to_empty_maps_and_does_not_panic() {
        ...
        let template = parse_nodepool_template("isolated-nodepool", &node_pool_data)
            .expect("spec present, must parse");
        assert_eq!(template.spec, serde_json::json!({"requirements": []}));
        assert!(
            template.labels.is_empty(),
            "a nil metadata block yields empty labels, not a panic"
        );
        assert!(
            template.annotations.is_empty(),
            "a nil metadata block yields empty annotations, not a panic"
        );
    }
    ...
    fn parse_nodepool_template_missing_spec_still_errors_the_same_as_before() {
        ...
        let err = parse_nodepool_template("isolated-nodepool", &node_pool_data)
            .expect_err("missing spec must surface, never be silently defaulted");
    ...
    async fn observe_computes_used_and_capacity_from_mocked_nodes_and_pods() {
        ...
        let p = KarpenterProvedor::new(
            env,
            "isolated-agents".into(),
            "isolated-nodepool".into(),
            live_gate(),
        );
    ...
    async fn per_node_alloc_milli_is_the_mean_over_owned_nodes_and_floors_to_one_when_empty() {
        ...
        let p_empty = KarpenterProvedor::new(
            MockEnv::empty(),
            "pool".into(),
            "nodepool".into(),
            live_gate(),
    ...
    async fn provision_dry_run_reports_would_and_creates_nothing() {
        ...
        assert!(
            p.env.created.lock().unwrap().is_empty(),
            "dry-run must create zero NodeClaims"
        );
    }
    ...
    async fn provision_live_creates_n_nodeclaims_copying_the_template_spec_verbatim() {
        let spec =
            serde_json::json!({"requirements": [{"key": "k", "operator": "In", "values": ["v"]}]});
        let env = MockEnv {
            template: Ok(NodePoolTemplate {
                spec: spec.clone(),
                ..NodePoolTemplate::default()
        ...
        assert_eq!(
            ...
            ProvisionReceipt::Applied {
                delta: 3,
                plan_id: "karpenter:provision:isolated-agents".into()
            }
        );
    ...
    async fn provision_live_missing_template_spec_propagates_the_error_and_creates_nothing() {
        let env = MockEnv {
            template: Err(ProviderError::ApiPermanent(
                "NodePool isolated-nodepool has no spec.template.spec".into(),
            )),
            ..MockEnv::empty()
        };
        let p = KarpenterProvedor::new(env, "pool".into(), "isolated-nodepool".into(), live_gate());
        let err = p
            .provision(2)
            .await
            .expect_err("a missing template spec must surface, never be silently skipped");
        assert!(matches!(err, ProviderError::ApiPermanent(_)));
    ...
    async fn provision_live_partial_create_failure_reports_the_actual_applied_count() {
        ...
        assert_eq!(
            ...
            ProvisionReceipt::Applied {
                delta: 2,
                plan_id: "karpenter:provision:pool".into()
            }
        );
    ...
    async fn deprovision_live_deletes_n_claims_in_deterministic_sorted_order() {
        ...
        assert_eq!(
            ...
            ProvisionReceipt::Applied {
                delta: -2,
                plan_id: "karpenter:deprovision:pool".into()
            }
        );
        assert_eq!(
            *p.env.deleted.lock().unwrap(),
            vec!["aaa".to_string(), "mmm".to_string()]
        );
    }
    ...
    fn claim_is_busy_true_only_when_node_name_is_in_the_busy_set() {
        ...
        let on_busy_node = NodeClaimRef {
            name: "c".into(),
            pool_label: Some("pool".into()),
            node_name: Some("node-a".into()),
        };
        let on_idle_node = NodeClaimRef {
            name: "c".into(),
            pool_label: Some("pool".into()),
            node_name: Some("node-b".into()),
        };
        let unregistered = NodeClaimRef {
            name: "c".into(),
            pool_label: Some("pool".into()),
            node_name: None,
        };
        ...
        let env = MockEnv {
            managed_claims: vec![
                NodeClaimRef {
                    name: "aaa".into(),
                    pool_label: Some("pool".into()),
                    node_name: Some("node-busy".into()),
                },
                NodeClaimRef {
                    name: "mmm".into(),
                    pool_label: Some("pool".into()),
                    node_name: Some("node-idle".into()),
                },
            ],
        ...
        let env = MockEnv {
            managed_claims: vec![
                NodeClaimRef {
                    name: "aaa".into(),
                    pool_label: Some("pool".into()),
                    node_name: Some("node-busy-1".into()),
                },
                NodeClaimRef {
                    name: "bbb".into(),
                    pool_label: Some("pool".into()),
                    node_name: Some("node-busy-2".into()),
                },
            ],
            busy_nodes: ["node-busy-1".to_string(), "node-busy-2".to_string()]
                .into_iter()
                .collect(),
            ..MockEnv::empty()
        };
    ...
    async fn deprovision_degrades_to_treating_all_claims_idle_when_observe_busy_fails() {
        ...
        impl KarpenterEnvironment for FailingBusyEnv {
            ...
            async fn observe_busy_node_names(&self) -> Result<HashSet<String>, ProviderError> {
                Err(ProviderError::ApiTransient(
                    "mock observe_busy_node_names failure".into(),
                ))
            }
        }
        let inner = MockEnv {
