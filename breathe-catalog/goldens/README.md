...
## `private-estate-global-breathe.golden.yaml`

The canonical `global.breathe` value block for the private estate posture — the render of
`preset::SPOT_AGGRESSIVE` by `render::render_global_breathe`. Pinned by
...
in the deployment chart's `architectures/the private estate.yaml`.
The chart's `global.breathe.preset: the private estate` names this source (the kata binding);
the explicit `memory` / `cpu` / `replica` bands under it are this render's
materialization. To change the private estate breathe posture, edit `src/preset.rs`
...
`tests/private-estate-breathe-parity_test.yaml` both fail until they agree.
...
cluster today. Live band emission + reaping the live orphan `private-estate-rabbitmq`
