...
impl QuinhaoPoolSpec {
    ...
    ///
    /// # Errors
    /// A [`QuinhaoPoolError`] for a malformed spec (the controller surfaces it in
    /// `status.reason`). A malformed FOREST (cycle/dup/unknown-parent) is NOT an
    /// error here — it folds into `status.reason` with `phase: Refused` so the
    /// receipt is observable.
    #[must_use]
